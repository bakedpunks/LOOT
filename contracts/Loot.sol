 ...
 */
contract Loot is Context,  AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage{
  ...
  function setBaseURI(string memory baseURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Loot: must have admin role to change base URI");
    _baseTokenURI = baseURI;
  }

  function setTokenURI(uint256 tokenId, string memory _tokenURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Loot: must have admin role to change token URI");
    _setTokenURI(tokenId, _tokenURI);
  }

  function setPrice(uint mintPrice) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Loot: must have admin role to change price");
    _price = mintPrice;
  }
  ...
  function mint() public payable {
    require(msg.value == _price, "Loot: must send correct price");
    require(_tokenIdTracker.current() < _max, "Loot: all Loot have been minted");
  ...
  function claimReward(uint256 tokenId) public {
    require(ownerOf(tokenId) == _msgSender(), "Loot: Only owner can claim rewards");
  ...
  function splitBalance(uint256 amount) private {
      uint256 reflectionShare = amount/32;
